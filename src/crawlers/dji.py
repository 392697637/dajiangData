    ...
    def _create_flyzone_table(self, conn):
        ...
        try:
            with conn.cursor() as cur:
                ...
                # 创建空间索引：先判断是否存在，存在则删除，再重新创建。
                geom_index_name = f"{self.db_table}_geom_idx"
                cur.execute(
                    """
                    SELECT EXISTS (
                        SELECT 1
                ...
                if cur.fetchone()[0]:
                    drop_index_sql = f'DROP INDEX "{geom_index_name}"'
                    cur.execute(drop_index_sql)
                    print(f"  删除旧空间索引: {geom_index_name}")
                ...
                cur.execute(
                    f'COMMENT ON TABLE "{self.db_table}" IS %s',
                    ("无人机禁飞区-大疆",),
                )
        ...
        try:
            ...
            insert_sql = f'''
                INSERT INTO "{self.db_table}" (
                    ...
                    shiid, shicode, shiname, shengcode, shengid, shengname, geom
                ) VALUES (
                    %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                    %s, %s, %s, %s, %s, %s, %s,
                    %s, %s, %s, %s, %s, %s,
