...
class AmapPOICrawler(BaseCrawler):
    ...
    def _get_region_code(self, region_name_cn):
        ...
        try:
            ...
            # 先查询市级表
            cur.execute("SELECT shicode, shiname FROM jc_shi WHERE shiname = %s LIMIT 1", (region_name_cn,))
            row = cur.fetchone()
            if row:
                return {'type': 'shi', 'code': row[0], 'name': row[1]}
            
            # 查询省级表
            cur.execute("SELECT shengcode, shengname FROM jc_sheng WHERE shengname = %s LIMIT 1", (region_name_cn,))
            row = cur.fetchone()
            if row:
                ...
                while not success and retry_count < max_retries:
                    ...
                    if data.get('status') != '1':
                        ...
                        if error_info == 'CUQPS_HAS_EXCEEDED_THE_LIMIT':
                            ...
                            print(f"⚠️ 触发限流，等待 {retry_delay} 秒后重试 (第 {retry_count + 1}/{max_retries} 次)")
    ...
    def _fetch_pois_polygon(self, polygon, keywords=None):
        ...
        if type_count:
            print("\n📊 当前网格POI类型统计:")
            for type_code, count in sorted(type_count.items(), key=lambda x: x[1], reverse=True):
    ...
    def crawl_region(self, region_name="河南省", grid_size_m=None, keywords=None, output_file=None):
        ...
        if type_stats:
            print("\n按类型统计:")
            for type_code, count in sorted(type_stats.items(), key=lambda x: x[1], reverse=True):
                type_name = self._get_type_name(type_code)
        ...
        return self._build_result_by_type(unique_pois, region_name, region_name_cn, grid_size_m, 
                                         grid_points, success_count, fail_count, 
                                         len(all_pois), keywords, output_file, region_level)

    def _build_result_by_type(self, pois, region_name, region_name_cn, grid_size_m, 
                              grid_points, success_count, fail_count, total_before_dedup, 
                              keywords, output_file, region_level=None):
        """
        ...
        for poi in pois:
            ...
            if region_level == 'city':
                if poi_city == region_name_cn or poi_city.startswith(region_name_cn[:-1]):
                    filtered_pois.append(poi)
            # 如果是省级区域（如河南省），只保留province匹配的POI
            elif region_level == 'province':
                if poi_province == region_name_cn or poi_province.startswith(region_name_cn[:-1]):
                    filtered_pois.append(poi)
            # 默认不过滤
            else:
                filtered_pois.append(poi)
        ...
        if region_code_info:
            ...
            print(f"✅ 从数据库查询到区域代码: {region_code_info['name']} ({region_code_info['type']}): {region_code}")
        else:
        ...
        if keywords is None:
            ...
            saved = self._save_pois_to_db_with_comment(filtered_pois, "amap", table_comment, {
                "mode": "region",
        ...
        # 有具体搜索关键词时，存入带keywords后缀的表
            ...
            self.db_table = table_name
            saved = self._save_pois_to_db(filtered_pois, "amap", {
                "mode": "region",
                "region": region_name_cn,
                "grid_size_km": grid_size_km,
                "keywords": keywords,
    ...
    def _save_pois_to_db_with_comment(self, pois, platform, table_comment, metadata=None):
        """
        将POI列表写入数据库，包含表注释
