...
-- 公共依赖
-- =============================================================================
-- 执行本校验脚本前，按以下顺序执行公共脚本：
--   1. PGSQL/函数/baseFunction/gis_drop_function.sql
--      创建 public.gis_drop_function(text)，供本脚本重建校验函数。
--   2. PGSQL/函数/baseFunction/gis_geojson_to_geom.sql
--      创建 public.gis_geojson_to_geom(text)，供本脚本解析 GeoJSON 空间数据。
--
-- 本脚本只创建/重建：
--   1. gis_validate_write_error
--   2. gis_validate_table
    ...
    FOR rec IN EXECUTE v_sql LOOP
        ...
        BEGIN
            ...
            ELSE
                ...
                v_geo_text := rec.geo_text;
                v_geom := gis_geojson_to_geom(v_geo_text);
            END IF;
            ...
            IF ST_GeometryType(v_geom) IN ('ST_Polygon', 'ST_MultiPolygon') THEN
                ...
                IF NOT v_is_ring_closed THEN
                    ...
                    v_correct_geom := gis_geojson_to_geom(v_geo_text);
