...
-- =============================================================================
-- 函数介绍：gis_dem_validate
-- 主要作用：校验河南 DEM 入库后的表、SRID、范围和电子围栏覆盖情况
-- 入参说明：p_old_srid、p_new_srid 为兼容旧调用参数，正式流程固定使用 4326
-- 返回说明：返回校验状态、DEM SRID、瓦片数量、4326 范围和围栏相交数量
-- 注意事项：正式 DEM 表固定读取 public.gis_dem_henan
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_elevation
-- 主要作用：根据经纬度查询 DEM 第一波段高程
-- 入参说明：p_lon 为经度；p_lat 为纬度；p_srid 为输入坐标系，默认 4326
-- 返回说明：返回高程值，点不在 DEM 覆盖范围内返回 NULL
-- 注意事项：经纬度顺序必须为 lon、lat
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_elevation_by_point
-- 主要作用：根据 Point 几何查询 DEM 第一波段高程
-- 入参说明：p_geom 为点几何，SRID 为空时按 4326 处理
-- 返回说明：返回高程值，点不在 DEM 覆盖范围内返回 NULL
-- 注意事项：仅支持 Point，其他几何类型返回 NULL
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_stats_by_polygon
-- 主要作用：统计指定面范围内 DEM 高程分布
-- 入参说明：p_geom 为 Polygon 或 MultiPolygon 几何
-- 返回说明：返回像元数量、最小高程、最大高程、平均高程和标准差
-- 注意事项：输入几何会自动转换到 DEM 表 SRID 后参与裁剪统计
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_profile_by_line
-- 主要作用：沿线路采样并查询每个采样点 DEM 高程
-- 入参说明：p_line 为 LineString；p_step 为采样间距，单位跟输入线一致
-- 返回说明：返回序号、线内距离、采样点和高程
-- 注意事项：4326 坐标单位为度，如需按米采样应先做米制坐标处理
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_point_with_elevation
-- 主要作用：给 Point 几何补充 DEM 高程，返回 PointZ
-- 入参说明：p_point 为点几何，SRID 为空时按 4326 处理
-- 返回说明：返回带 Z 值的点几何
...
-- =============================================================================
-- 函数介绍：gis_dem_line_with_elevation
-- 主要作用：给 LineString 每个顶点补充 DEM 高程，返回 LineStringZ
-- 入参说明：p_line 为线几何，SRID 为空时按 4326 处理
-- 返回说明：返回带 Z 值的线几何
-- 注意事项：仅处理 LineString，MultiLineString 需拆分后调用
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_polygon_with_elevation
-- 主要作用：给 Polygon 外环和内环顶点补充 DEM 高程，返回 PolygonZ
-- 入参说明：p_polygon 为面几何，SRID 为空时按 4326 处理
-- 返回说明：返回带 Z 值的面几何
-- 注意事项：仅处理 Polygon，MultiPolygon 需拆分后调用
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_dem_geom_with_elevation
-- 主要作用：根据输入几何类型统一补充 DEM 高程
-- 入参说明：p_geom 支持 Point、LineString、Polygon
-- 返回说明：返回带 Z 值的几何，不支持的类型返回 NULL
-- 注意事项：复杂几何集合需先拆分为单一几何后调用
-- =============================================================================
