-- =============================================================================
-- 4.建筑自动生成.sql
--   gis_generate_building_3d                生成建筑三维体块
--   exec_shell_cmd_capture                  执行命令并返回结果
--   gis_generate_3dtiles                    生成建筑三维瓦片文件
--
-- =============================================================================
...
-- =============================================================================
-- 函数介绍：gis_generate_building_3d
-- 主要作用：根据项目建筑二维面和高度字段，生成项目建筑三维体块数据表。
-- 入参说明：p_project_id 为项目ID，用于定位gis_buildings_<project_id>等项目建筑数据。
-- 返回说明：返回执行状态、生成表名和处理数量，供三维瓦片生成或路径避障使用。
-- 注意事项：依赖PostGIS三维几何处理；建筑高度缺失时需按函数内默认规则处理。
-- =============================================================================
...
-- ============================================================
-- =============================================================================
-- 函数介绍：exec_shell_cmd_capture
-- 主要作用：在数据库侧调用系统命令，并捕获命令输出结果。
-- 入参说明：cmd 为需要执行的系统命令文本。
-- 返回说明：返回命令输出文本，供外部工具链调用结果记录或调试。
-- 注意事项：该函数涉及服务器命令执行权限，生产环境需严格控制调用权限和命令来源。
-- =============================================================================
...
-- ============================================================
-- =============================================================================
-- 函数介绍：gis_generate_3dtiles
-- 主要作用：把项目建筑三维数据转换生成3DTiles瓦片文件，供三维地图前端加载。
-- 入参说明：传入项目ID和输出目录或相关路径参数，用于定位建筑数据和生成目标位置。
-- 返回说明：返回生成状态、输出路径、执行日志等信息，便于接口侧判断瓦片生成结果。
-- 注意事项：依赖外部三维瓦片转换工具和服务器文件权限；调用前需先生成建筑三维体块。
-- =============================================================================
