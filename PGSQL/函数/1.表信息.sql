...
--
-- 本文件定位：
--   这是整个无人机 GIS 路径规划模块的“数据字典/模板脚本”，负责定义各类项目表的标准字段、
--   索引策略和字段语义。3.1/3.2/3.3 中的建表、打标、规划函数都以这里的字段约定为基础。
--
-- 推荐执行顺序：
--   1. 先执行本文件，确保模板表、公共结果表和字段注释存在。
--   2. 再执行 3.1，生成项目级三维网格并做电子围栏/建筑打标。
--   3. 再执行 3.2，创建全局粗规划函数。
--   4. 如需要“100m粗规划 + 20/30m走廊精规划”，再执行 3.3。
--
-- 顺序说明：
-- 1. 三维网格节点表模板：系统空间计算底座
-- 2. 网格打标数据源：按 block_mask 顺序排列
--    1=电子围栏，2=建筑，4=地形，8=倾斜摄影，16=DEM，32=国家空域规则
--    任一阻塞位命中时 is_flyable=false；全部阻塞位清空后 is_flyable=true。
--    路径规划函数优先读取 is_flyable，不再逐个判断各来源字段。
...
--
-- 设计原则：
--   1. 路径规划只直接依赖网格表中的 is_flyable/block_mask/geom/xyz 等轻量字段。
--   2. 电子围栏、建筑、地形、DEM 等复杂空间数据先通过打标函数压缩为 block_mask。
--   3. A* 搜索阶段尽量不直接扫描原始海量障碍数据，避免每次规划都做昂贵空间连接。
--   4. 大数据表优先使用项目隔离表，减少跨项目扫描、锁竞争和索引膨胀。
-- ==============================================
