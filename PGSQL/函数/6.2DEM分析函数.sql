...
--
-- 返回策略:
--   1. 本文件函数直接返回业务结果，不统一封装 code/msg。
--   2. 查询不到 DEM、点不在 DEM 覆盖范围内或无有效像元时，按函数语义返回 NULL 或空结果。
--   3. 输入类型不符合要求、DEM 表不存在等明确错误，会通过 RAISE EXCEPTION 抛出。
--   4. 若需要接口统一 code/msg，可在服务层将正常结果映射为 code=200，
--      参数或空间条件不满足映射为 code=400，数据库异常映射为 code=500。
-- =============================================================================
...
-- 返回值：
--   regclass       DEM 表对象；解析不到有效表时返回 NULL。
-- =============================================================================
...
-- 返回值：
--   double precision 高程值；点不在 DEM 覆盖范围内或 DEM 表不可用时返回 NULL。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(p_point) <> 'ST_Point' THEN
        RAISE EXCEPTION 'gis_dem_value_at_point 只支持 Point，当前类型: %', ST_GeometryType(p_point);
    END IF;

    v_dem_reg := public.gis_dem_resolve_table(p_point, p_dem_table);
...
-- 返回值：
--   TABLE(dem_table, dem_srid, tile_count, geom)。
-- =============================================================================
...
BEGIN
    ...
    IF v_dem_reg IS NULL THEN
        RAISE EXCEPTION 'DEM表不存在: %', p_dem_table;
    END IF;
...
-- 返回值：
--   TABLE(pixel_count, min_elevation, max_elevation, mean_elevation, stddev_elevation)。
-- =============================================================================
...
BEGIN
    ...
    v_dem_reg := public.gis_dem_resolve_table(p_geom, p_dem_table);
    IF v_dem_reg IS NULL THEN
        RAISE EXCEPTION 'DEM表不存在: %', p_dem_table;
    END IF;
    ...
    IF ST_GeometryType(v_geom) NOT IN ('ST_Polygon', 'ST_MultiPolygon') THEN
        RAISE EXCEPTION 'gis_dem_stats_by_polygon 只支持 Polygon/MultiPolygon，当前类型: %', ST_GeometryType(v_geom);
    END IF;
...
-- 返回值：
--   TABLE(class_min, class_max, pixel_count)。
-- =============================================================================
...
--   2. p_dem_table 可选 DEM 表名；为空时按 p_geom 自动解析 DEM 表。
-- 返回值：
--   TABLE(point_type, elevation, geom)，point_type 为 min 或 max。
-- =============================================================================
CREATE OR REPLACE FUNCTION public.gis_dem_minmax_points_by_polygon(
    p_geom geometry,
    p_dem_table text DEFAULT NULL
)
RETURNS TABLE(point_type text, elevation double precision, geom geometry(Point, 4326))
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
    ...
    v_geom geometry;
BEGIN
    IF p_geom IS NULL OR ST_IsEmpty(p_geom) THEN RETURN; END IF;
    v_dem_reg := public.gis_dem_resolve_table(p_geom, p_dem_table);
    IF v_dem_reg IS NULL THEN RAISE EXCEPTION 'DEM表不存在: %', p_dem_table; END IF;
...
--   2. p_dem_table 可选 DEM 表名；为空时按 p_geom 自动解析 DEM 表。
-- 返回值：
--   TABLE(min_elevation, max_elevation, relative_height)。
-- =============================================================================
CREATE OR REPLACE FUNCTION public.gis_dem_relative_height_by_polygon(
    p_geom geometry,
    p_dem_table text DEFAULT NULL
)
RETURNS TABLE(min_elevation double precision, max_elevation double precision, relative_height double precision)
LANGUAGE sql
STABLE
AS $$
    SELECT min_elevation, max_elevation, max_elevation - min_elevation
    FROM public.gis_dem_stats_by_polygon(p_geom, p_dem_table);
$$;
...
--   3. p_dem_table        可选 DEM 表名；为空时按 p_geom 自动解析 DEM 表。
-- 返回值：
--   TABLE(grid_id, geom, min_elevation, max_elevation, relative_height)。
-- =============================================================================
...
RETURNS TABLE(grid_id bigint, geom geometry(Polygon, 4326), min_elevation double precision, max_elevation double precision, relative_height double precision)
LANGUAGE sql
STABLE
AS $$
    WITH input AS (
    ...
    SELECT g.grid_id, g.geom, r.min_elevation, r.max_elevation, r.relative_height
    FROM clipped_grid g
...
--   2. p_dem_table 可选 DEM 表名；为空时按 p_geom 自动解析 DEM 表。
-- 返回值：
--   TABLE(min_elevation, max_elevation, mean_elevation, relative_height, risk_level)。
-- =============================================================================
CREATE OR REPLACE FUNCTION public.gis_dem_elevation_risk_by_polygon(
    p_geom geometry,
    p_dem_table text DEFAULT NULL
)
RETURNS TABLE(min_elevation double precision, max_elevation double precision, mean_elevation double precision, relative_height double precision, risk_level text)
LANGUAGE sql
STABLE
AS $$
    SELECT min_elevation, max_elevation, mean_elevation, max_elevation - min_elevation,
        CASE
            WHEN max_elevation IS NULL THEN '未知'
            WHEN max_elevation - min_elevation >= 500 THEN '高起伏'
            WHEN max_elevation - min_elevation >= 200 THEN '中起伏'
            WHEN max_elevation - min_elevation >= 50 THEN '低起伏'
            ELSE '平缓'
        END
    FROM public.gis_dem_stats_by_polygon(p_geom, p_dem_table);
$$;
...
RETURNS TABLE(seq bigint, geom geometry(Point, 4326), elevation double precision)
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
...
AS $$
    ...
    SELECT p.seq, p.geom, public.gis_dem_value_at_point(p.geom, p_dem_table)
    FROM pts p, input i
...
-- 返回值：
--   TABLE(seq, distance_m, geom, elevation)。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(v_line_4326) <> 'ST_LineString' THEN
        RAISE EXCEPTION 'gis_dem_profile_by_line 只支持 LineString，当前类型: %', ST_GeometryType(v_line_4326);
    END IF;
    ...
    SELECT
        ...
        d.distance_m,
        d.geom,
        public.gis_dem_value_at_point(d.geom, p_dem_table) AS elevation
    FROM dedup d
...
--   3. p_dem_table 可选 DEM 表名；为空时按 p_line 自动解析 DEM 表。
-- 返回值：
--   TABLE(sample_count, min_elevation, max_elevation, mean_elevation, total_ascent, total_descent)。
-- =============================================================================
...
RETURNS TABLE(sample_count bigint, min_elevation double precision, max_elevation double precision, mean_elevation double precision, total_ascent double precision, total_descent double precision)
LANGUAGE sql
STABLE
AS $$
    WITH p AS (
...
--   3. p_dem_table 可选 DEM 表名；为空时按 p_line 自动解析 DEM 表。
-- 返回值：
--   TABLE(distance_m, start_elevation, end_elevation, max_slope_percent, total_ascent, total_descent)。
-- =============================================================================
...
RETURNS TABLE(distance_m double precision, start_elevation double precision, end_elevation double precision, max_slope_percent double precision, total_ascent double precision, total_descent double precision)
LANGUAGE sql
STABLE
AS $$
    WITH p AS (
...
--   3. p_dem_table 可选 DEM 表名；为空时按 p_line 自动解析 DEM 表。
-- 返回值：
--   jsonb，包含 seq、distance_m、elevation、geometry。
-- =============================================================================
...
-- 返回值：
--   TABLE(elevation, slope_degree, slope_percent, aspect_degree, aspect_name)。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(p_point) <> 'ST_Point' THEN
        RAISE EXCEPTION 'gis_dem_slope_aspect_at_point 只支持 Point，当前类型: %', ST_GeometryType(p_point);
    END IF;
    ...
    zc := public.gis_dem_value_at_point(v_point, p_dem_table);
    zn := public.gis_dem_value_at_point(ST_Project(v_point::geography, v_d, radians(0))::geometry, p_dem_table);
    ze := public.gis_dem_value_at_point(ST_Project(v_point::geography, v_d, radians(90))::geometry, p_dem_table);
    zs := public.gis_dem_value_at_point(ST_Project(v_point::geography, v_d, radians(180))::geometry, p_dem_table);
    zw := public.gis_dem_value_at_point(ST_Project(v_point::geography, v_d, radians(270))::geometry, p_dem_table);

    IF zn IS NULL OR zs IS NULL OR ze IS NULL OR zw IS NULL THEN
        RETURN QUERY SELECT zc, NULL::double precision, NULL::double precision, NULL::double precision, NULL::text;
        RETURN;
    END IF;
...
--   3. p_dem_table 可选 DEM 表名；为空时按 p_point 自动解析 DEM 表。
-- 返回值：
--   TABLE(elevation, slope_degree, slope_percent, risk_level)。
-- =============================================================================
...
RETURNS TABLE(elevation double precision, slope_degree double precision, slope_percent double precision, risk_level text)
LANGUAGE sql
STABLE
AS $$
    SELECT elevation, slope_degree, slope_percent,
        CASE
            WHEN slope_degree IS NULL THEN '未知'
            WHEN slope_degree < 5 THEN '平缓'
            WHEN slope_degree < 15 THEN '缓坡'
            WHEN slope_degree < 30 THEN '陡坡'
            ELSE '极陡坡'
        END
    FROM public.gis_dem_slope_aspect_at_point(p_point, p_sample_m, p_dem_table);
$$;
...
-- 返回值：
--   TABLE(ground_elevation, observer_elevation, target_height_m, geom)。
-- =============================================================================
...
RETURNS TABLE(ground_elevation double precision, observer_elevation double precision, target_height_m double precision, geom geometry(Point, 4326))
LANGUAGE sql
STABLE
AS $$
    SELECT
        public.gis_dem_value_at_point(p_point, p_dem_table),
        public.gis_dem_value_at_point(p_point, p_dem_table) + COALESCE(p_observer_height_m, 0),
...
-- 返回值：
--   TABLE(seq, distance_m, geom, flight_altitude, ground_elevation, clearance_m)。
-- =============================================================================
...
RETURNS TABLE(seq integer, distance_m double precision, geom geometry(Point, 4326), flight_altitude double precision, ground_elevation double precision, clearance_m double precision)
LANGUAGE sql
STABLE
AS $$
    WITH line2d AS (
...
-- 返回值：
--   TABLE(seq, distance_m, geom, clearance_m, risk_level)。
-- =============================================================================
...
RETURNS TABLE(seq integer, distance_m double precision, geom geometry(Point, 4326), clearance_m double precision, risk_level text)
LANGUAGE sql
STABLE
AS $$
    SELECT seq, distance_m, geom, clearance_m,
        CASE
            WHEN clearance_m IS NULL THEN '未知'
            WHEN clearance_m < 0 THEN '撞地风险'
            WHEN clearance_m < p_safe_clearance_m THEN '低净空'
            ELSE '安全'
        END
    FROM public.gis_dem_obstacle_clearance_by_line(p_line_z, p_step_m, p_dem_table)
    WHERE clearance_m IS NULL OR clearance_m < p_safe_clearance_m;
$$;
...
BEGIN
    ...
    RETURN format('等高线表生成完成: %s, interval=%s', v_target, p_interval);
END;
$$;
...
--   1. p_dem_table DEM 栅格表名；默认 public.gis_dem_henan。
-- 返回值：
--   TABLE(dem_table, dem_srid, tile_count, pixel_size_x, pixel_size_y, extent)。
-- =============================================================================
CREATE OR REPLACE FUNCTION public.gis_dem_water_flow_prepare(
    p_dem_table text DEFAULT 'public.gis_dem_henan'
)
RETURNS TABLE(dem_table text, dem_srid integer, tile_count bigint, pixel_size_x double precision, pixel_size_y double precision, extent geometry)
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
...
BEGIN
    ...
    IF v_dem_reg IS NULL THEN RAISE EXCEPTION 'DEM表不存在: %', p_dem_table; END IF;
    RETURN QUERY EXECUTE format($sql$
        SELECT %L::text, ST_SRID(rast), COUNT(*)::bigint, AVG(abs(ST_ScaleX(rast))), AVG(abs(ST_ScaleY(rast))),
...
-- 返回值：
--   geometry，返回 PointZ 或 MultiPointZ。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(ST_Force2D(p_point)) NOT IN ('ST_Point', 'ST_MultiPoint') THEN
        RAISE EXCEPTION 'gis_dem_point_with_elevation 只支持 Point/MultiPoint，当前类型: %', ST_GeometryType(p_point);
    END IF;
    RETURN public.gis_dem_elevation_base(p_point, p_dem_table);
END;
$$;
...
--   2. p_dem_table 可选 DEM 表名；为空时由 6.1 核心函数自动解析。
-- 返回值：
--   geometry，返回 LineStringZ 或 MultiLineStringZ。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(ST_Force2D(p_line)) NOT IN ('ST_LineString', 'ST_MultiLineString') THEN
        RAISE EXCEPTION 'gis_dem_line_with_elevation 只支持 LineString/MultiLineString，当前类型: %', ST_GeometryType(p_line);
    END IF;
    RETURN public.gis_dem_elevation_base(p_line, p_dem_table);
END;
$$;
...
--   2. p_dem_table 可选 DEM 表名；为空时由 6.1 核心函数自动解析。
-- 返回值：
--   geometry，返回 PolygonZ 或 MultiPolygonZ。
-- =============================================================================
...
BEGIN
    ...
    IF ST_GeometryType(ST_Force2D(p_polygon)) NOT IN ('ST_Polygon', 'ST_MultiPolygon') THEN
        RAISE EXCEPTION 'gis_dem_polygon_with_elevation 只支持 Polygon/MultiPolygon，当前类型: %', ST_GeometryType(p_polygon);
    END IF;
    RETURN public.gis_dem_elevation_base(p_polygon, p_dem_table);
END;
$$;
...
--   2. p_dem_table 可选 DEM 表名；为空时由 6.1 核心函数自动解析。
-- 返回值：
--   geometry，返回与输入类型对应的带 Z geometry。
-- =============================================================================
...
AS $$
BEGIN
    RETURN public.gis_dem_elevation_base(p_geom, p_dem_table);
END;
$$;
...
-- SELECT public.gis_dem_resolve_table(
--   ST_SetSRID(ST_MakePoint(113.65, 34.76), 4326),
--   NULL
-- );
--
-- 1.2 gis_dem_value_at_point：查询单点 DEM 高程。
-- SELECT public.gis_dem_value_at_point(
--   ST_SetSRID(ST_MakePoint(113.65, 34.76), 4326)
-- );
--
-- 1.3 gis_dem_extent：查询 DEM 覆盖范围、SRID 和瓦片数量。
-- SELECT dem_table, dem_srid, tile_count, ST_AsGeoJSON(geom) AS geom_geojson
...
-- SELECT point_type, elevation, ST_AsGeoJSON(geom) AS geom_geojson
...
--
-- 2.5 gis_dem_terrain_relief_by_grid：按规则网格统计地形起伏度。
-- SELECT grid_id, min_elevation, max_elevation, relative_height, ST_AsGeoJSON(geom) AS geom_geojson
...
-- SELECT seq, elevation, ST_AsGeoJSON(geom) AS geom_geojson
...
-- SELECT public.gis_dem_cross_section_geojson(
--   ST_GeomFromText('LINESTRING(113.6 34.7,113.8 34.9)', 4326),
--   100
-- );
...
-- SELECT public.gis_dem_generate_contour_table(
--   50,
